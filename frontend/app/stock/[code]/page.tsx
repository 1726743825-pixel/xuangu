    ...
    <div className="mx-auto w-full max-w-[1480px] px-4 py-7 sm:px-6 lg:px-8">
      ...
      <header className="mt-5 rounded-2xl border border-slate-200/80 bg-white px-5 py-5 shadow-sm dark:border-slate-800 dark:bg-slate-900 sm:px-6">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.18em] text-indigo-600"><span className="h-2 w-2 rounded-full bg-indigo-500" />Stock detail</div><div className="flex flex-wrap items-center gap-x-3 gap-y-1"><h1 className="text-3xl font-bold tracking-tight text-slate-950 dark:text-white">{detail.data?.name ?? "加载中…"}</h1><span className="font-mono text-sm text-slate-400">{code}</span><span className="text-sm text-slate-500 dark:text-slate-400">{detail.data?.industry ?? "未分类"}</span></div></div>
          <div className="flex items-end gap-4"><span className="font-mono text-3xl font-bold tracking-tight text-slate-950 dark:text-white">{formatNumber(quote?.price)}</span><span className={`mb-1 font-mono text-base font-semibold ${change == null || change === 0 ? "text-slate-500" : change > 0 ? "text-red-500" : "text-emerald-600"}`}>{change == null ? "—" : `${change > 0 ? "+" : ""}${change.toFixed(2)}%`}</span></div>
        </div>
      </header>
