...
// ── 自动更新配置 ──
const SCRIPT_VERSION = '2.3.0';          // 当前版本号
const UPDATE_URL = 'https://gist.githubusercontent.com/1726743825-pixel/be9bfb4c401fbe9a6d10e56b344c2875/raw/97f087476f41b5f6036b974d92d0c738d9650cff/stock_screener.js';
const AUTO_UPDATE_CHECK = true;          // 是否启用自动更新检查
...
async function getMarketData() {
  const url = 'https://push2.eastmoney.com/api/qt/stock/get?secid=1.000001&fields=f43,f170,f113,f114,f58';
  ...
  const fall = d.f114 || 0;
  return {
    sseChangePercent: d.f170 / 100,   // 涨跌幅%: -226 → -2.26
    price: d.f43 / 100,                // 最新价: 402726 → 4027.26
    riseCount: rise,
    fallCount: fall,
    advDeclRatio: fall > 0 ? rise / fall : 0,  // 涨跌比
  };
}
...
async function getSectorList() {
  const url = 'https://push2.eastmoney.com/api/qt/clist/get?cb=&pn=1&pz=500&po=1&np=1&fltt=2&invt=2&fs=m:90+t:2&fields=f12,f14,f3,f86,f87';
  const j = await httpGetJSON(url);
  if (!j || !j.data || !j.data.diff) return null;
  return j.data.diff.map(item => ({
    bkCode: item.f12,           // 如 BK0459
    name: item.f14,             // 如 "元件"
    changePercent: item.f3,     // 板块涨跌幅%
    riseCount: item.f86 || 0,   // 上涨家数
    fallCount: item.f87 || 0,   // 下跌家数
  }));
}
...
async function getSectorConstituents(sectorCode) {
  const url = `https://push2.eastmoney.com/api/qt/clist/get?cb=&pn=1&pz=200&po=1&np=1&fltt=2&invt=2&fs=b:${sectorCode}&fields=f12,f14,f3`;
  const j = await httpGetJSON(url);
  if (!j || !j.data || !j.data.diff) return null;
  return j.data.diff.map((item, idx) => ({
    rank: idx + 1,
    code: item.f12,
    name: item.f14,
...
async function getStockDetail(code) {
  ...
  const url = `https://push2.eastmoney.com/api/qt/stock/get?secid=${market}.${code}&fields=f57,f58,f62,f184,f66,f187,f170`;
  const j = await httpGetJSON(url);
  if (!j || !j.data) return null;
  ...
  return {
    code: d.f57,
    name: d.f58,
    mainNetInflow: d.f62,              // 主力净流入(元)
    mainForcePercent: d.f184,           // 主力净流入占比%
    largeOrderPercent: d.f187 === '-' ? null : d.f187, // 大单净流入占比%
    changePercent: d.f170 != null ? d.f170 / 100 : null, // 涨跌幅%
  };
}
...
async function getZTPremium() {
  const url = 'https://push2.eastmoney.com/api/qt/clist/get?cb=&pn=1&pz=100&po=1&np=1&fltt=2&invt=2&fs=b:BK0815&fields=f12,f3';
  const j = await httpGetJSON(url);
  if (!j || !j.data || !j.data.diff) return null;

  const stocks = j.data.diff;
  let total = 0, count = 0;
  for (const s of stocks) {
    if (s.f3 != null && !isNaN(s.f3)) { total += s.f3; count++; }
  }
 ...
 */
async function buildSectorInfo(candidates) {
  const industries = await getSectorList();
  if (!industries) return null;

  const nameToBK = {};
  for (const ind of industries) nameToBK[ind.name] = ind.bkCode;

  const bkCache = {};
  const result = {};

  for (const stock of candidates) {
    const indName = stock.industry;
    if (!indName || !nameToBK[indName]) continue;

    const bkCode = nameToBK[indName];
    const indInfo = industries.find(x => x.bkCode === bkCode);
    if (!indInfo) continue;

    // 缓存成分股
    if (!bkCache[bkCode]) {
      bkCache[bkCode] = await getSectorConstituents(bkCode);
      await sleep(500); // v2.3.0: 防反爬
    }
    const constituents = bkCache[bkCode];
    if (!constituents) continue;

    const sectorRank = industries.findIndex(x => x.bkCode === bkCode) + 1;
    const sectorRankPercent = (sectorRank / industries.length) * 100;
    ...
    result[stock.code] = {
      ...
      stockRankPercent,
      sectorChangePercent: indInfo.changePercent,
    };
  }
...
async function checkDragonTigerRetail(code) {
  const url = `https://datacenter-web.eastmoney.com/api/data/v1/get?reportName=RPT_DMSK_TSLS&columns=ALL&filter=(SECURITY_CODE%3D%22${code}%22)`;
  const j = await httpGetJSON(url);
  if (!j?.result?.data) return null;
  const buySeats = j.result.data.filter(d => d.TRADE_TYPE === '买入');
  const retailCount = buySeats.filter(s => RETAIL_SEATS.some(kw => s.BRANCH_NAME?.includes(kw))).length;
  if (retailCount >= 3) return { name: '龙虎榜散户霸榜', score: -3, detail: `买入前5中拉萨系占${retailCount}席` };
  return null;
}
...
async function checkAnnouncementRisk(code) {
  ...
  const url = `https://searchapi.eastmoney.com/api/silverselect/web/queryAnnouncement?code=${market}${code}&pageSize=10`;
  const j = await httpGetJSON(url);
  if (!j?.data) return null;
  const fiveDaysAgo = new Date(Date.now() - 5 * 86400000);
  for (const item of j.data) {
    if (new Date(item.noticeDate) < fiveDaysAgo) continue;
    const title = item.title || '';
    if (title.includes('减持') || title.includes('解禁') || title.includes('减持计划')) {
      return { name: '股东减持/解禁', score: -5, detail: `${title.includes('解禁')?'解禁':'减持'}:${title.slice(0,20)}` };
    }
  }
  return null;
}
...
async function getCandidates() {
  ...
  return { candidates: merged, yesterdayPool };
}
...
function calcBonus(klines, stock, todaySSE, sectorInfo) {
  let bonusTotal = 0;
  let bonusItems = [];
  ...
  // ── 确定性加分 (最高 15 分) — 月度动态更新 ──
  // 4维度12指标，每月初手动更新以下配置，由 AI Agent 或人工填入最新政策/资金/基本面/产业数据
  // ============================================================
  // 【使用说明】每月初更新下面 scores 数组中的 value 值（0~max）
  // 数据来源：国务院/部委政策文件、公募基金季报、公司财报、行业研报
  // value=0 表示该指标无加分，留空或填0即可
  // ============================================================
...
`);
  ...
  // 2. 获取板块热点数据（批量一次性获取）
  console.log('📡 获取板块数据...');
  const sectorInfoMap = await buildSectorInfo(toAnalyze);
  ...
  for (let i = 0; i < toAnalyze.length; i++) {
    ...
    // 计算加分项
    const { bonusTotal, bonusItems } = calcBonus(klines, stock, todaySSE, sectorInfo);
